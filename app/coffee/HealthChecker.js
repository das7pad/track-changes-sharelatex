...
module.exports = {
	check(callback){
		...
		const jobs = [
			cb => request.get({url:`http://localhost:${port}/check_lock`, timeout:3000}, function(err, res, body) {
                if (err != null) {
			...
			cb => request.post({url:`${url}/flush`, timeout:10000}, function(err, res, body) {
                if (err != null) {
			...
			cb => request.get({url:`${url}/updates`, timeout:10000}, function(err, res, body){
                if (err != null) {
