...
module.exports = (PackManager = {
	...
	archivePack(project_id, doc_id, pack_id, callback) {
		...
		return async.series([
			...
			cb => MongoAWS.archivePack(project_id, doc_id, pack_id, err => clearFlagOnError(err, cb)),
			cb => PackManager.checkArchivedPack(project_id, doc_id, pack_id, err => clearFlagOnError(err, cb)),
	...
	processOldPack(project_id, doc_id, pack_id, callback) {
		const markAsChecked = err => PackManager.markPackAsChecked(project_id, doc_id, pack_id, function(err2) {
